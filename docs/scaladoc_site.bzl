...
def _scaladoc_site_impl(ctx):
    # Scaladoc exits successfully without rendering when it has no TASTy or JAR input.
    bootstrap = ctx.attr.scaladoc[JavaInfo].outputs.jars[0].class_jar
...
scaladoc_site = rule(
    ...
    attrs = {
        ...
        "repository_url": attr.string(
            default = "https://github.com/lqhuang/scala-binding-foundation",
        ),
        "scaladoc": attr.label(
            cfg = "exec",
            executable = True,
...
def _scaladoc_site_test_impl(ctx):
    ...
    ctx.actions.write(
        content = "#!/usr/bin/env bash\nset -euo pipefail\ntest -f \"${TEST_SRCDIR}/${TEST_WORKSPACE}/%s/index.html\"\n" % site.short_path,
        is_executable = True,
        output = executable,
    )
    return [DefaultInfo(
