 ...
 *
 * [[FromResult]] and [[ToResult]] define conversions between `Result` and
...
object Result:
  ...
  extension [T, E](result: Result[T, E]) {
    ...
    /**
     * Returns the value from this `Ok` or the given `default` argument if this
     * is a `Err`.
     *
     * ''Note:'': This will throw an err if it is not a success and default
     * throws an err.
     */
    ...
    /**
     * Applies the given function `f` if this is a `Ok`, otherwise returns
     * `Unit` if this is a `Err`.
     */
    ...
    /**
     * Maps the given function to the value from this `Ok` or returns this if
     * this is a `Err`.
     */
     ...
     * given predicate `p` holds for the right value, or `Failture(zero)` if
     * this is a `Ok` and the given predicate `p` does not hold for the right
     * value, or `Failture` with the existing value of `Failture` if this is a
     * `Failture`.
     */
    ...
    /**
     * Completes this `Result` by applying the function `f` to this if this is
     * of type `Err`, or conversely, by applying `s` if this is a `Ok`.
     */
    ...
    /**
     * Applies the given function `f` if this is a `Err`, otherwise returns this
     * if this is a `Ok`. This is like `flatMap` for the exception.
     *
     * ===Examples===
     *
     * {{{
    ...
    /**
     * Applies the given function `f` if this is a `Err`, otherwise returns this
     * if this is a `Ok`. This is like map for the exception.
     *
    ...
    /**
     * `joinFailure` is analogous to `joinLeft` for consistency with `Either`
     * API
     *
     * @group Transform
     */
 ...
 *
 * This interface is used by the [[Result.from]] method.
 */
