 ...
 * exception that was thrown. This is similar to the [[redef.data.Result]] type,
 * but with different semantics.
 *
 * A `Try[T]` is either an [[Ok]][T] or an [[Err]][Throwable].
 *
...
object Try:
  ...
  extension [T](result: Try[T]) {
    ...
    def get: T = result match {
      ...
      case Err(exception) => throw exception
    }
    ...
    def collect[U](pf: PartialFunction[T, U]): Try[U] = result match {
      case Err(exception) => result.asInstanceOf[Try[U]]
      case Ok(value)      =>
      ...
      case Ok(value) =>
        try
          ...
          then result
            ...
            Err(
              new NoSuchElementException(
                s"Predicate does not hold for ${value}"
              )
            )
        catch case NonFatal(exc) => Err(exc)
    }

    /**
     * Creates a non-strict filter, which eventually converts this to a
     * `Failure` if the predicate is not satisfied.
     *
...
object SafeTry:
  ...
  def apply[T, E <: Exception](r: => T throws E): SafeTry[T, E] =
    try
      val r1 = r
      Ok(r1)
    catch case e: E => Err(e)

  def ok[T, E <: Exception](value: T): SafeTry[T, E] = Ok(value)

  def failure[T, E <: Exception](err: E): SafeTry[T, E] = Err(err)
